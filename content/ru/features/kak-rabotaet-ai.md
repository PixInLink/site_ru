  ...
  <div class="row">
    <div class="col-12 col-md-10 offset-md-1">
      ...
      <p class="use-text-subtitle2 text-muted" style="font-size:1.2rem;line-height:1.6;">
        Полный разбор пайплайна: GigaChat, CDN-кеш, WebP-оптимизация и 152-ФЗ
      </p>
      <div class="card paper pa-4 mt-4" style="background:linear-gradient(135deg,#e8eaf6 0%,#e3f2fd 100%);border:1px solid #c5cae9;border-radius:12px;">
        <p class="body-1 mb-0" style="line-height:1.85;">
          ...
          генерирует через <strong>GigaChat</strong> →
          оптимизирует WebP → сохраняет в Yandex Object Storage →
    ...
    <div class="col-12 col-md-10 offset-md-1">
      <div class="card paper pa-4 mb-5" style="background:#f8fff7;border-left:4px solid #43a047;box-shadow:none;">
        ...
        <p class="body-1 m-0" style="line-height:1.9;font-family:monospace;font-size:14px;">
          ...
          &nbsp;&nbsp;• Кеша нет: SVG placeholder → Celery → GigaChat<br>
          &nbsp;&nbsp;&nbsp;&nbsp;→ WebP → S3 → CDN (140 POP) → Redis<br>
          Первый запрос = <strong>10–30 секунд</strong> (генерация).<br>
      ...
      <div class="row mb-5">
        <div class="col-md-6 col-lg-3 mb-3">
          <div class="card paper pa-4 text-center" style="height:100%;border-top:4px solid #3949ab;">
            <div style="font-size:28px;font-weight:700;color:#3949ab;">10–30 сек</div>
            <p class="body-2 text-muted mb-0">P95 latency первой генерации через GigaChat</p>
          </div>
        </div>
        <div class="col-md-6 col-lg-3 mb-3">
        ...
        <div class="col-md-6 col-lg-3 mb-3">
          <div class="card paper pa-4 text-center" style="height:100%;border-top:4px solid #e53935;">
            <div style="font-size:28px;font-weight:700;color:#e53935;">Многопоточно</div>
            <p class="body-2 text-muted mb-0">6 параллельных генераций GigaChat одновременно</p>
          </div>
        </div>
        <div class="col-md-6 col-lg-3 mb-3">
      ...
      <div class="row mb-5">
        <div class="col-md-6 mb-3">
          <div class="card paper pa-4" style="height:100%;border-left:4px solid #3949ab;">
            ...
            <p class="body-2 m-0" style="line-height:1.75;">Цепочка обработки запроса: от текстового промпта до готового изображения. Включает нейросетевую генерацию, постобработку, загрузку в хранилище и CDN-доставку. В PixInLink — автоматический, асинхронный, с многопоточной генерацией.</p>
          </div>
        </div>
        <div class="col-md-6 mb-3">
          <div class="card paper pa-4" style="height:100%;border-left:4px solid #e53935;">
            ...
            <p class="body-2 m-0" style="line-height:1.75;">Российская нейросеть от Сбера, способная генерировать изображения по текстовому описанию. Обучена на русскоязычных данных — лучше понимает кириллические промпты, чем Midjourney или DALL-E. Основная AI-модель в PixInLink.</p>
          </div>
        </div>
        <div class="col-md-6 mb-3">
        ...
        <div class="col-md-6 mb-3">
          <div class="card paper pa-4" style="height:100%;border-left:4px solid #00838f;">
            ...
            <p class="body-2 m-0" style="line-height:1.75;"><strong>Cache miss:</strong> изображения нет в кеше → нужна генерация (10–30 сек). <strong>Cache hit:</strong> изображение уже создано → CDN отдаёт мгновенно (&lt;100 мс), квота не расходуется. После первой генерации все повторные запросы — cache hit.</p>
          </div>
        </div>
        <div class="col-md-6 mb-3">
...
<div class="container" id="how-ai-works-intro">
  <div class="row">
    <div class="col-12 col-md-10 offset-md-1">
      ...
      <p class="body-1 mb-4" style="line-height:1.85;">
        Если вы впервые запрашиваете изображение, браузер покажет SVG-заглушку на 10–30 секунд,
        пока нейросеть GigaChat рисует картинку. Это нормально: <strong>холодный старт</strong>.
...
<div class="container" id="how-ai-works-pipeline">
  <div class="row">
    <div class="col-12 col-md-10 offset-md-1">
      <h2 class="display-1 mb-4" style="font-size:24px;font-weight:700;">9 шагов от URL до изображения: полный пайплайн</h2>
      <!-- Шаг 1 -->
      <div class="card paper pa-4 mb-3" style="border-left:4px solid #3949ab;">
        ...
        <p class="body-2 mt-2 mb-0 text-muted">Валидация: width/height 16–8192, prompt не пустой. Если что-то не так → <code>400 Bad Request</code>.</p>
      </div>
      <!-- Шаг 2 -->
...
→ abc123def456...</pre>
      ...
      <div class="card paper pa-4 mb-3" style="border-left:4px solid #00838f;">
        ...
        <p class="body-2 mb-0">API сразу возвращает SVG-заглушку (200 OK, ~200 байт) с заголовком <code>X-Job-Id</code>. Браузер не ждёт 10–30 секунд — видит заглушку немедленно. Параллельно API создаёт запись в PostgreSQL (<code>status=pending</code>) и отправляет задачу в очередь Celery.</p>
      </div>
      <!-- Шаг 5 -->
      ...
      <!-- Шаг 6: AI-генерация GigaChat -->
      <div class="card paper pa-4 mb-3" style="border-left:4px solid #e53935;">
        <div class="d-flex align-items-center mb-2">
          <div style="background:#e53935;color:#fff;font-weight:700;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-right:12px;flex-shrink:0;font-size:14px;">6</div>
          <div>
            <h3 class="headline mb-0" style="font-size:16px;font-weight:700;">AI-генерация: GigaChat</h3>
        ...
        <div class="pa-3 mb-2" style="background:#e8eaf6;border-radius:8px;">
          ...
          <p class="body-2 mb-0">Генерация изображений по текстовому промпту. Обучена на русскоязычных данных — лучше понимает кириллические промпты, чем зарубежные модели. Хостинг: SberCloud (Россия, 152-ФЗ). Поддерживает многопоточную генерацию: 6 параллельных запросов одновременно через пул Celery-воркеров.</p>
        </div>
        <div class="mt-2 pa-2" style="background:#e8f5e9;border-radius:6px;">
          <p class="body-2 m-0">💡 <strong>Многопоточная генерация:</strong> 3 GPU-воркера × concurrency=2 = 6 изображений генерируются одновременно. При пиковой нагрузке система автоматически масштабируется, добавляя новые воркеры.</p>
        </div>
      </div>
      <!-- Шаг 7: Постобработка -->
      <div class="card paper pa-4 mb-3" style="border-left:4px solid #00838f;">
        <div class="d-flex align-items-center mb-2">
          <div style="background:#00838f;color:#fff;font-weight:700;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-right:12px;flex-shrink:0;font-size:14px;">7</div>
          <div>
...
Durability: 99.999999999%</pre>
      ...
      <div class="card paper pa-4 mb-5" style="border-left:4px solid #43a047;">
        <div class="d-flex align-items-center mb-2">
          <div style="background:#43a047;color:#fff;font-weight:700;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-right:12px;flex-shrink:0;font-size:14px;">9</div>
          <div>
    ...
    F --> G["Celery Worker"]
    G --> H["GigaChat — SberCloud 10–30 сек"]
    H --> I["Image Processor Pillow: resize, WebP, watermark"]
    I --> J["Yandex Object Storage S3 immutable"]
    J --> K["Yandex CDN 140 POP"]
    K --> L["Redis UPDATE image:{hash}"]
    L --> M["Следующий запрос → Cache HIT &lt;100 мс ✅"]</pre>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="row">
    <div class="col-12 col-md-10 offset-md-1">
      ...
      <p class="body-1 mb-4" style="line-height:1.85;">
        ...
        управляемого текстовым описанием. GigaChat не «выбирает» — он «рисует».
      </p>
      <div class="card paper pa-0 mb-4 overflow-hidden">
        <div class="table-responsive">
          <table class="highlight m-0">
  ...
  <div class="row">
    <div class="col-12 col-md-10 offset-md-1">
      ...
      <div class="row mb-4">
        <div class="col-md-4 mb-3">
          <div class="card paper pa-4" style="height:100%;border-top:4px solid #3949ab;text-align:center;">
            ...
            <h4 class="headline mb-2" style="font-size:15px;font-weight:700;">GigaChat (primary)</h4>
            <p class="body-2 m-0">SberCloud API, Россия. При 5xx или timeout → автоматический retry через Celery.</p>
          </div>
        </div>
        <div class="col-md-4 mb-3">
          <div class="card paper pa-4" style="height:100%;border-top:4px solid #fb8c00;text-align:center;">
            ...
            <h4 class="headline mb-2" style="font-size:15px;font-weight:700;">Многопоточная генерация</h4>
            <p class="body-2 m-0">3 GPU-воркера × concurrency=2 = 6 параллельных потоков GigaChat. Пиковая нагрузка автоматически распределяется между воркерами.</p>
          </div>
        </div>
        <div class="col-md-4 mb-3">
      ...
      <div class="card paper pa-0 mb-4 overflow-hidden">
        <div class="table-responsive">
          <table class="highlight striped m-0">
            ...
            <tbody class="body-2">
              <tr><td>GigaChat API</td><td>502/503/timeout</td><td>Retry → другой GPU-воркер</td><td style="color:#2e7d32;">Нет (чуть медленнее)</td></tr>
              <tr><td>GPU worker</td><td>OOM/crash</td><td>Retry → другой воркер</td><td style="color:#2e7d32;">Нет</td></tr>
      ...
      <div class="card paper pa-3 mb-5" style="background:#f3f8ff;border:1px solid #bbdefb;border-radius:8px;">
        <p class="body-2 m-0">
          <strong>Circuit Breaker:</strong> при 5 подряд ошибках GigaChat → Circuit Breaker открывается на 60 сек.
          Все запросы ставятся в очередь. После 60 сек — пробный запрос к GigaChat. Если OK — Circuit Breaker закрывается.
        </p>
      </div>
    </div>
  </div>
        ...
        <div class="table-responsive">
          <table class="highlight striped m-0">
            ...
            <tbody class="body-2">
              ...
              <tr><td>AI Primary</td><td>GigaChat</td><td>SberCloud, Россия</td></tr>
  ...
  <div class="row">
    <div class="col-12 col-md-10 offset-md-1">
      ...
      <div class="card paper pa-4 mb-4" style="background:#f3f8ff;border:1px solid #bbdefb;border-radius:8px;">
        ...
        <p class="body-2 mb-2">Параметр <code>&seed={число}</code> задаёт начальное значение для генератора случайных чисел. Представьте, что генерация изображения — это бросок огромного количества многогранных кубиков. <code>&seed</code> — это, по сути, «запись» того, как именно эти кубики упали.</p>
        <p class="body-2 mb-2"><strong>Воспроизводимость:</strong> один и тот же промпт + один и тот же seed = всегда идентичное изображение. Это позволяет не просто получить хороший результат, но и вернуться к нему в любой момент.</p>
        <p class="body-2 mb-0"><strong>Контролируемые вариации:</strong> если вам нравится композиция, но вы хотите изменить цвет или деталь — оставьте тот же seed и поменяйте только часть промпта. GigaChat сохранит общую структуру изображения, изменив только указанный аспект.</p>
      </div>
      ...
      <div class="row mb-4">
        ...
        <div class="col-md-4 mb-3">
          <div class="card paper pa-4" style="height:100%;border-top:4px solid #43a047;">
            ...
            <p class="body-2 mb-0">Стабильный CDN-кеш. Идеально для og:image и заголовков статей. Один seed = одно изображение навсегда.</p>
          </div>
        </div>
        <div class="col-md-4 mb-3">
...
# seed=847392145</pre>
      ...
      <div class="card paper pa-4 mb-5" style="background:#fff8e1;border-left:4px solid #ffb300;">
        <p class="body-2 m-0">
          <strong>Пример контролируемой вариации:</strong> вам понравилось изображение с запросом <code>офис-разработчиков-неон?style=cyberpunk&seed=12345</code>, но вы хотите дневную версию. Меняете промпт на <code>офис-разработчиков?style=cyberpunk&seed=12345</code> — GigaChat сохранит композицию, но уберёт неон и ночную атмосферу.
        </p>
      </div>
    </div>
  </div>
            ...
            <ul class="collapsible">
              <li class="accordion-content paper active">
                ...
                <div class="collapsible-body detail">
                  <p>Cache miss — нейросеть GigaChat рисует специально для этого запроса. P95 = 10–30 секунд. Все повторные запросы отдаются из CDN: &lt;100 мс. Квота при cache hit не расходуется.</p>
                </div>
              </li>
              <li class="accordion-content paper">
                ...
                <div class="collapsible-body detail">
                  <p>Нет. Cache hit не расходует квоту. 500 генераций на Starter — это 500 уникальных промптов. Один промпт можно запрашивать любое количество раз — квота тратится 1 раз при первой генерации.</p>
                </div>
              </li>
              <li class="accordion-content paper">
                <div class="collapsible-header content">
                ...
                <div class="collapsible-body detail">
                  <p>GigaChat — российская нейросеть от Сбера. Обучена на русскоязычных данных, лучше понимает кириллические промпты, чем зарубежные модели. Хостится в России (152-ФЗ). Поддерживает многопоточную генерацию — до 6 изображений одновременно.</p>
                </div>
              </li>
              <li class="accordion-content paper">
                <div class="collapsible-header content">
                  <p class="heading">Изображения хранятся вечно?</p>
                  <i class="material-icons right arrow">expand_more</i>
                </div>
                <div class="collapsible-body detail">
                  <p>CDN-кеш = 1 год. В Yandex Object Storage: 180 дней → Cold Storage, 365 дней → удаление (lifecycle policy). После удаления следующий запрос = новая генерация.</p>
                </div>
              </li>
              <li class="accordion-content paper">
                <div class="collapsible-header content">
                ...
                <div class="collapsible-body detail">
                  <p>При одинаковом prompt + seed — да, CDN отдаст тот же кешированный файл. Изображение уникально создано для этого промпта. Хотите гарантированную уникальность — не указывайте seed (random каждый раз).</p>
                </div>
              </li>
              <li class="accordion-content paper">
                <div class="collapsible-header content">
                ...
                <div class="collapsible-body detail">
                  <p>GigaChat имеет встроенный content policy filter. Запросы с NSFW, насилием и запрещённым контентом отклоняются. PixInLink дополнительно проверяет на уровне API Gateway.</p>
                </div>
              </li>
              <li class="accordion-content paper">
                <div class="collapsible-header content">
                ...
                <div class="collapsible-body detail">
                  <p>Да, на платных тарифах (без watermark) — полные коммерческие права. На Free (с watermark) — личное использование. Подробнее: <a href="https://pixinlink.ru/terms/">pixinlink.ru/terms</a>.</p>
                </div>
              </li>
              <li class="accordion-content paper">
                <div class="collapsible-header content">
                  <p class="heading">Сколько GPU-ресурсов на одну генерацию?</p>
                  <i class="material-icons right arrow">expand_more</i>
                </div>
                <div class="collapsible-body detail">
                  <p>GigaChat API — GPU на стороне SberCloud. Собственные GPU-воркеры: RTX 4090, 24 GB VRAM, concurrency=2 на worker. 3 воркера = 6 параллельных генераций. При высокой нагрузке (utilization &gt;85%) auto-scale добавляет воркеры автоматически.</p>
                </div>
              </li>
            </ul>
          </div>
    ...
    <div class="col-12 col-md-10 offset-md-1">
      ...
      <div class="card paper pa-4 mb-5" style="background:#fafafa;border:1px solid #e0e0e0;border-radius:8px;">
        <p class="body-1 mb-3" style="line-height:1.85;">
          ...
          ADR-010 (Watermark). Мониторинг 24/7 через Grafana + Telegram-алерты.
        </p>
