...
<!-- @block: answer-first -->
**Промпт = субъект + детали + сцена + освещение + стиль.** Это не просто «описание картинки» — это инструкция для нейросети на русском языке. Кириллица для Kandinsky 3.1 родная. 5 слов дают случайный результат, 50–100 — предсказуемый, 100–300 — идеальный. Параметр `?style=` выносится из текста промпта в URL и задаётся отдельно.
<!-- @block: key-facts -->
...
<div class="col-md-4 col-sm-12 pa-2">
  <div class="card paper pa-3" style="border-left:4px solid var(--color-accent)">
    ...
    <p class="use-text-paragraph mb-0 small">Время первой генерации. Все последующие загрузки — мгновенно из кеша CDN. 12 генераций бесплатно без регистрации.</p>
  </div>
</div>
</div>
...
<p>Скопируйте URL ниже, подставьте свой промпт — и откройте в браузере. 12 генераций бесплатно, без регистрации. Результат через 15 секунд.</p>
